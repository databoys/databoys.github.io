...
self.ah = tanh(sum)
```
When we calculate the gradient for the tanh we will just use the new tanh derivative that we defined earlier in the place of the logistic sigmoid derivative. 
``` python
...
```
Again, I am skipping the math in these posts and just focusing on readable python code and a higher level of understanding. But, essentially what happened is that if you were to work out the gradient descent algorithm with the derivative of the softmax function you will end up cancelling and just ending up with -(t - yhat) for the error calculation, where t is the true value and yhat is the predicted value. Awesome!
